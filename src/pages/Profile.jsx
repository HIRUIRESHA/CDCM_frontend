...
const Profile = () => {
  ...
  const baseUrl = user?.role === 'PATIENT' 
    ? `http://localhost:8082/api/patient/${user?.id}`
    : `http://localhost:8082/api/doctors/${user?.id}`;
  ...
  const handleSave = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // This instantly updates the AuthContext, changing the Sidebar icon
        updateUser({
    ...
    <div className="max-w-3xl mx-auto p-8">
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Add other fields (Email, Phone, etc.) here following the same pattern */}
        </div>
