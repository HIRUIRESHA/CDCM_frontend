...
export default function DoctorNotifications() {
  const { user } = useAuth(); // Get the logged-in doctor
  const [notifications, setNotifications] = useState([]);
  const [loading, setLoading] = useState(true);

  // Fetch notifications whenever the logged-in doctor changes
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      try {
        setLoading(true);
        const res = await getNotifications(doctorId); // fetch by AuthContext doctor ID
